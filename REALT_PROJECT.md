...
## ✅ Реализованный функционал (v0.4.2)
...
- **Привязка к ЖК** — кнопка "🧮 Расчёт" подставляет данные объекта

### Поиск
- Через RAG — семантический поиск по документам
- В контексте ЖК или по всей базе
- Fallback на прямой поиск если RAG пустой
...
- [ ] **Этап 2:** Универсальный handler (убираем FSM)
- [ ] **Этап 3:** Генерация документов (LLM → HTML → PDF)
- [ ] **Этап 4:** Калькуляторы как функции LLM
