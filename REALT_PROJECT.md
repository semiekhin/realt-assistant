# Realt Assistant — Документация проекта

## 🎯 Что это

**Realt Assistant** — Telegram-бот для риэлторов. Персональный AI-ассистент для работы с базой ЖК.

**Концепция**: Риэлтор загружает документы по ЖК → бот парсит и понимает ВСЁ → риэлтор общается естественным языком, как с Claude.ai.

**Для кого**: Индивидуальные риэлторы, работающие "на ходу" с разными застройщиками.

---

## 📍 Инфраструктура

| Компонент | Значение |
|-----------|----------|
| Сервер | `72.56.64.91:2222` (SSH) |
| Путь | `/opt/realt-assistant` |
| Telegram бот | `@RealtAssistentAI_bot` |
| GitHub | https://github.com/semiekhin/realt-assistant |
| Python | 3.12+ |
| База | SQLite (`data/assistant.db`) |

**Другие сервисы на сервере (НЕ ТРОГАТЬ)**:
- `/opt/bot` — RIZALTA PROD (порт 8000)
- `/opt/bot-dev` — RIZALTA DEV (порт 8002)
- `/opt/sofia-claude`, `/opt/sofia-gpt`

---

## 🚀 Запуск и управление
```bash
# Запуск
cd /opt/realt-assistant
/opt/realt-assistant/venv/bin/python -u run_polling.py > bot.log 2>&1 &

# Проверка статуса
ps aux | grep "realt-assistant" | grep -v grep

# Остановка
pkill -f "realt-assistant.*run_polling"

# Логи
tail -f /opt/realt-assistant/bot.log

# Перезапуск
pkill -f "realt-assistant.*run_polling" && sleep 1 && cd /opt/realt-assistant && /opt/realt-assistant/venv/bin/python -u run_polling.py > bot.log 2>&1 &
```

---

## 📁 Структура проекта
```
/opt/realt-assistant/
├── app.py                     # FastAPI webhook + роутинг
├── run_polling.py             # Запуск в polling режиме
├── config.py                  # Конфигурация
├── requirements.txt
├── .env                       # Секреты (не в git)
│
├── docs/
│   └── RAG_CORE_SPEC.md       # ТЗ на RAG-ядро (утверждено)
│
├── bot/
│   ├── states.py              # FSM состояния
│   └── handlers/
│       ├── start.py           # /start, меню
│       ├── add_property.py    # Добавление ЖК
│       ├── query.py           # Поиск, просмотр ЖК
│       ├── kp.py              # Генерация КП с выбором стиля
│       └── calculators.py     # Калькуляторы + привязка к ЖК
│
├── services/
│   ├── telegram.py            # Telegram API
│   ├── llm.py                 # OpenAI API + промпт извлечения данных
│   ├── parser.py              # Старый парсер (не используется)
│   ├── parser_v2.py           # Vision-first парсер (PDF → картинки → GPT-4o)
│   ├── calculators.py         # Расчёты рассрочки/ипотеки/ROI
│   ├── content_composer.py    # AI-генерация контента для документов
│   ├── kp_generator_v2.py     # PDF рендеринг с динамическими стилями
│   ├── pdf_styles.py          # 6 стилей оформления
│   └── fonts/                 # DejaVuSans для PDF
│
├── db/
│   ├── database.py            # SQLite операции
│   └── models.py              # Dataclass модели + to_summary/to_full_info
│
└── data/
    ├── assistant.db           # SQLite база
    ├── uploads/               # Загруженные файлы
    └── kp_output/             # Сгенерированные PDF
```

---

## ✅ Реализованный функционал (v0.4.1)

### Управление ЖК
- Добавление ЖК с автопарсингом файлов
- **Vision-first парсинг** — PDF как изображения через GPT-4o
- Полная карточка для проверки риэлтором
- Просмотр списка ЖК
- Карточка ЖК с документами
- Удаление ЖК

### Парсинг файлов (parser_v2.py)
- **PDF → изображения страниц → Vision API**
- Excel (pandas)
- DOCX (python-docx + таблицы)
- Изображения (GPT-4o Vision)
- LLM извлекает структурированные данные с запретом галлюцинаций

### Генерация КП
- Content Composer — AI создаёт продающий контент
- 6 стилей: premium, business, corporate, modern, minimal, warm
- Выбор стиля кнопками

### Калькуляторы
- **Рассрочка** — расчёт ежемесячного платежа
- **Ипотека** — 4 программы (стандартная, семейная, IT, господдержка)
- **ROI** — доходность инвестиций
- **Привязка к ЖК** — кнопка "🧮 Расчёт" подставляет данные объекта

### Поиск
- В контексте ЖК: "что есть до 15 млн?"
- Глобальный по всей базе

---

## 🗄 База данных
```sql
-- users
id, telegram_id, username, first_name, last_name, state, state_data, created_at

-- properties  
id, user_id, name, address, city, developer, completion_date,
price_min, price_max, price_per_sqm_min, price_per_sqm_max,
apartment_types, area_min, area_max,
payment_options, installment_terms, mortgage_info,
installment_min_pv, installment_max_months, installment_markup,
distance_to_sea, territory_area, hotel_operator,
commission, description, features, raw_data, 
created_at, updated_at

-- property_files
id, property_id, user_id, file_id, file_name, file_type, file_path, extracted_text, created_at
```

---

## 🔮 Будущее: RAG-ядро

**Утверждено ТЗ:** docs/RAG_CORE_SPEC.md

**Концепция:** Бот = персональный Claude для риэлтора
- Загрузил документы — спрашивай что угодно
- LLM сам понимает, сам решает, сам делает
- Никаких жёстких меню, роутеров, интентов
- Адаптивная генерация документов (LLM → HTML → PDF)

**Этапы:**
1. RAG ядро (ChromaDB, embeddings)
2. Универсальный handler (убираем FSM)
3. Генерация документов (LLM → HTML → PDF)
4. Калькуляторы как функции LLM
5. Помощник переписки с клиентами

---

## 📋 История версий

### v0.4.1 (08.01.2026)
- Калькулятор привязан к ЖК (кнопка "🧮 Расчёт")
- Утверждено ТЗ на RAG-ядро (docs/RAG_CORE_SPEC.md)

### v0.4.0 (07.01.2026 вечер)
- **Vision-first парсер** — PDF как изображения через GPT-4o
- Запрет галлюцинаций в промпте LLM
- Новые поля: distance_to_sea, territory_area, hotel_operator
- Полная карточка ЖК (to_full_info)

### v0.3.0 (07.01.2026)
- Калькуляторы: рассрочка, ипотека (4 программы), ROI
- Структурированные поля рассрочки
- Content Composer + 6 стилей PDF
- GitHub репозиторий

### v0.2.0 (07.01.2026)
- Content Composer — AI создаёт продающий контент
- 6 стилей PDF с выбором кнопками

### v0.1.0 (05.01.2026)
- Базовый бот с FSM
- Добавление/просмотр ЖК
- Парсинг файлов
- Простая генерация КП
