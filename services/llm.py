...
# === Промпт для извлечения данных о ЖК ===
...
async def extract_property_data(text: str, property_name: str = "") -> Optional[Dict[str, Any]]:
    """
    Извлечь структурированные данные о ЖК из текста
    
    Args:
        text: Объединённый текст из всех файлов
        property_name: Название ЖК (если известно)
    ...
    user_prompt += f"Материалы:\n\n{text[:15000]}"  # Ограничиваем размер
...
async def extract_text_from_image(image_base64: str) -> Optional[str]:
    """
    Извлечь текст из изображения через GPT-4 Vision
    """
    if not client:
        return None
...
async def answer_query(query: str, properties_context: str) -> str:
    """
    Ответить на вопрос пользователя по базе ЖК
    """
    if not client:
...
async def quick_chat(message: str, context: str = "") -> str:
    """
    Быстрый ответ без специального контекста
    """
    if not client:
        return "❌ Сервис временно недоступен"
